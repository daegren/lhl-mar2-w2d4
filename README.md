W2D4 - Promises
====
Big props to Martin & KV for the demo idea and notes 👊
...
See file `02_callbacks.js` for the solution.

### 3. Sync flow using Async tasks (with Promises)

We revisited the same File I/O problem using Promises. For this we used the `fs.promises` library ([link](https://nodejs.org/dist/latest-v10.x/docs/api/fs.html#fs_fs_promises_api))

We were able to avoid the callback hell/waterfall using promises properly.

See `03_promises.js` for the solution we wrote.
...
Take a look at `04_creating-promises.js` for an example of  how to use `new Promise((resolve, reject) => {})` to create new promises. These are useful as tools to combine several other async actions together into a single promise.
